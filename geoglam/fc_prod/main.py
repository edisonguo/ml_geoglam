...
def ml_superfunc(models, arr, mask):
    res = np.ones((arr.shape[0], 3), dtype=np.uint8) * 255

    res_masks = mask & (np.sum(arr[:, :7] < 0, axis=1) == 0) & (np.sum(arr[:, :7] > 1, axis=1) == 0)
    if res_masks.sum() == 0:
        return res

    valid_arr = arr[res_masks, :]

    for ib, model in enumerate(models):
        pred = model.predict(valid_arr)
        pred = np.clip(pred, 0, 254).astype(np.uint8)
        res[res_masks, ib] = pred
...
if __name__ == "__main__":
    ...
    parser.add_argument(dest="model_path", type=str, help="Root path")    
    ...
    n_models = 0 
    for p in os.listdir(args.model_path):
        if p.startswith('rf') and p.endswith('.model'):
            n_models += 1
    ...
    for i in range(n_models):
        model_file = os.path.join(args.model_path, 'rf%d.model' % i)
    ...
    for im, modis_tile in enumerate(tile_files):
        ...
        t0 = time.time()
        out = ml_superfunc(models, bands_stack, mask).reshape(2400, 2400, 3)
        print '    time ml_superfunc(): ', time.time() - t0
